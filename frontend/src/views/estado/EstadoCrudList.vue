          ...
          <v-card-text>
            <v-container>
              <v-row>
                ...
                <v-col cols="12" sm="12" md="12">
                  <v-text-field v-model="editedItem.abreviacao" label="Abreviação"></v-text-field>
                </v-col>
              </v-row>
            </v-container>
          </v-card-text>
...
export default {
  ...
  methods: {
    ...
    deleteItem (item) {
      confirm('Tem certeza que quer apagar esse estado?') && this.delete(item);
    },
    async delete(item) {
      try {
        await this.requestClass.remove(item._id);
        this.makeRestRequest();
        this.showSuccessToast({title: 'Estado removido com sucesso!'})
      } catch (error) {
  ...
  computed: {
    formTitle () {
      return (this.editedItem._id && this.editedItem._id !== undefined) ? 'Editar estado' : 'Cadastrar estado';
    },
  }
};
</script>
